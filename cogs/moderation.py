    ...
    async def clear(self, ctx, *, arg: str):
     ...
     if arg == "contains bots":
         ...
         return await ctx.send(
             f"🤖 Deleted **{len(deleted) - 1}** bot messages.",
             delete_after=3
         )
 
     # ---------------- KEYWORD ----------------
     if arg.startswith("contains "):
         keyword = arg.replace("contains ", "").strip()
         ...
         return await ctx.send(
             f"🔍 Deleted **{len(deleted) - 1}** messages containing **'{keyword}'**.",
             delete_after=3
         )
